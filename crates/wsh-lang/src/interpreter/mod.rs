...
impl Shell {
    ...
    fn run_wasm_func(
        &mut self,
        wasm_func: wsh_engine::WasmFuncUntyped,
        cmd: &Command,
        mut stdout: File,
        mut stderr: File,
        stdin: Option<os_pipe::PipeReader>,
        ...
        if let Err(err) = unsafe {
            ...
            writeln!(stderr, "wsh: error prepare WASI: {err}").expect("write to stderr failed!");
            return Err(());
        }
        ...
        if arg_types.len() != cmd.args.len() {
            writeln!(
                stderr,
                "wsh: expected {} args for wasm function",
                arg_types.len()
            )
        ...
        // We must convert wsh values to wasm values
        let args = cmd
            .args
            .iter()
            .zip(arg_types)
            .map(|(arg, ty)| -> ShellResult<wsh_engine::Value> {
                let mut to_val = |n| match ty {
                    ValType::I32 => Ok(wsh_engine::Value::I32(n as u32)),
                    ValType::I64 => Ok(wsh_engine::Value::I64(n as u64)),
                    ValType::F32 => Ok(wsh_engine::Value::F32(n as f32)),
                    ValType::F64 => Ok(wsh_engine::Value::F64(n)),
                    _ => {
                        writeln!(
                            stderr,
                            "wsh: could not convert {n} to {ty} for wasm function `{}`",
                            cmd.name
                        )
                        .expect("write to stderr failed!");
                        Err(())
                ...
                match self.eval_expr(arg)? {
                    ...
                    Value::String(_) => {
                        writeln!(
                            stderr,
                            "wsh: cannot pass string to wasm function `{}`",
                            cmd.name
                        )
                        .expect("write to stderr failed!");
                        Err(())
                    ...
                    Value::Bool(b) => match ty {
                        ValType::I32 => Ok(wsh_engine::Value::I32(b as u32)),
                        ValType::I64 => Ok(wsh_engine::Value::I64(b as u64)),
                        _ => {
                            writeln!(
                                stderr,
                                "wsh: could not convert bool to {ty} for wasm function `{}`",
                                cmd.name
                            )
                            .expect("write to stderr failed!");
                            Err(())
                    ...
                    Value::Null => {
                        writeln!(
                            stderr,
                            "wsh: cannot pass null to wasm function `{}`",
                            cmd.name
                        )
                        .expect("write to stderr failed!");
                        Err(())
        ...
        let results = match wasm_func.call(self.env.store_mut(), &args) {
            ...
            Err(err) => {
                writeln!(
                    stderr,
                    "wsh: error calling wasm function `{}`: {err}",
                    cmd.name
                )
                .expect("write to stderr failed!");
                return Err(());
    ...
    fn print_err(&mut self, err: ShellError) {
        let mut stderr = unsafe { OpenFileDescriptor::new(self.stderr) };
        writeln!(stderr, "wsh: {err}").expect("write to stderr failed!");
    }
